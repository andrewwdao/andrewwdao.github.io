...
(function () {
    ...
    if (shapeDefs === undefined) {
        shapeDefs = {
            ...
            ex: ['M', -0.3535, -0.3535, 'L', 0.3535, 0.3535, 'M', -0.3535, 0.3535, 'L', 0.3535, -0.3535],
        };
    }

    function DrawCmd(cmdStr, coords) {
        // canvas syntax draw commands
    ...
    function cgo2DtoDrawCmd(path, scl) {
        ...
        for (i = 0; i < segments.length; i++) {
            ...
            cmd = seg[0];
            if (i == 0 && cmd != 'M') {
                // check that the first move is absolute
                cmd = 'M';
            }
            ...
            switch (cmd) {
                ...
                case 'S':
                    ...
                    cmdObj = new DrawCmd('bezierCurveTo', [
                        x - (px - x),
                        y - (py - y),
                        xOfs + xScale * coords[0],
                        yOfs + yScale * coords[1],
                        xOfs + xScale * coords[2],
                        yOfs + yScale * coords[3],
                    ]);
                    commands.push(cmdObj);
                ...
                case 'Q':
                    ...
                    break;
                case 'q':
                    cmdObj = new DrawCmd('quadraticCurveTo', [
                        x + xScale * coords[0],
                        y + yScale * coords[1],
                        x + xScale * coords[2],
                        y + yScale * coords[3],
                ...
                case 'T':
                    ...
                    cmdObj = new DrawCmd('quadraticCurveTo', [px, py, xOfs + xScale * coords[0], yOfs + yScale * coords[1]]);
                    commands.push(cmdObj);
                    px = x - (px - x);
                    py = y - (py - y);
                    x = xOfs + xScale * coords[0];
                ...
                case 't':
                    ...
                    cmdObj = new DrawCmd('quadraticCurveTo', [px, py, x + xScale * coords[0], y + yScale * coords[1]]);
                    commands.push(cmdObj);
                    x += xScale * coords[0];
                    y += yScale * coords[1];
                    break;
    ...
    Obj2D.prototype.translate = function (x, y) {
        ...
        if (this.type == 'IMG' || this.type == 'TEXT') {
            ...
            for (
                j = 0;
                j < this.bBoxCmds.length;
                j++ // step through the draw segments
            ) {
    ...
    Obj2D.prototype.scale = function (xScl, yScl) {
        ...
        if (this.type == 'IMG' || this.type == 'TEXT') {
            ...
            if (this.type == 'PATH') {
                // let line thickness of Paths scale too
    ...
    Obj2D.prototype.appendPath = function (obj, delMove) {
        ...
        if (delMove) {
            // delete the inital 'moveTo' command
    ...
    Cango = function (canvasId) {
        ...
        this.cnvs.onmousedown = function (evt) {
            ...
            function hitTest(pathObj) {
                ...
                if (pathObj.type == 'TEXT' || pathObj.type == 'IMG') {
                    // use bounding box not drawCmds
...
*/
            ...
            for (
                j = len - 1;
                j >= 0;
                j-- // search last drawn first, it will be on top
            ) {
    ...
    Cango.prototype.render = function (pathObj, x, y, scl, degs) {
        ...
        function iterate(obj) {
            var x, childNode;
            if (isArray(obj)) {
                // array of Obj2D or mixed Obj2D and more arrays
                for (x = 0; x < obj.length; x++) {
                    childNode = obj[x];
                    if (childNode.drawCmds !== undefined) {
                        // test for Obj2Ds to draw
        ...
        for (i = 0; i < drawObjAry.length; i++) {
            if (drawObjAry[i].type == 'IMG') {
                if (drawObjAry[i].width > 0) {
                    // image loaded and width set?
    ...
    Cango.prototype._paintImg = function (pathObj, x, y, scl, degrees) {
        ...
        this.ctx.drawImage(
            img,
            this.xscl * scale * pathObj.imgX,
            this.xscl * scale * pathObj.imgY,
            this.xscl * scale * pathObj.width,
            this.xscl * scale * pathObj.height
        );
    ...
    Cango.prototype._paintPath = function (pathObj, x, y, scl, degrees) {
        ...
        for (i = 0; i < drawCmdsLen; i++) {
            ...
            for (
                j = 0;
                j < parmsLen;
                j += 2 // convert x,y coord pairs to pixel coords
            ) {
        ...
        for (i = 0; i < drawCmdsLen; i++) {
            ...
            for (
                j = 0;
                j < parmsLen;
                j += 2 // convert x,y coord pairs to world coords
            ) {
    ...
    Cango.prototype.compileText = function (str, color, fontSz, fontWt, lorigin) {
        var size = fontSz || this.fontSize, // fontSize in pts
            ...
            pathObj,
            wid,
            hgt, // Note: char cell is ~1.5* fontSize pixels high
            wid2,
            hgt2,
            lorgWC,
            dx,
        ...
        wid2 = wid / 2;
        hgt2 = hgt / 2;
        lorgWC = [0, [0, 0], [wid2, 0], [wid, 0], [0, hgt2], [wid2, hgt2], [wid, hgt2], [0, hgt], [wid2, hgt], [wid, hgt]];
        dx = -lorgWC[lorg][0];
        dy = -lorgWC[lorg][1];
    ...
    Cango.prototype.drawImg = function (imgURL, x, y, w, lorigin) {
        // just load img then calls _paintImg
        var savThis = this,
            xOfs = x || 0,
            yOfs = y || 0,
            lorg = lorigin || 1,
    ...
    Cango.prototype.clipPath = function (pathObj) {
        ...
        for (i = 0; i < pathObj.drawCmds.length; i++) {
            for (
                j = 0;
                j < pathObj.drawCmds[i].parms.length;
                j += 2 // convert x,y coord pairs to pixel coords
            ) {
    ...
    Cango.prototype.deleteLayer = function (ovlyId) {
        ...
        if (idx !== -1) {
            ...
            if (ovlNode) {
                // there is a id stored but no actual canvas
    ...
    function interpolate(vals, t, dur) {
        // vals is an array of key frame values
    ...
    function DeclAnimation(obj, xVals, yVals, sclVals, rotVals, delayTime, duration, repeat) {
        ...
        this.getState = function (time, state) {
            // reference to a state passed to avoid creating new objects
            var localTime, t;

            if (time == 0) {
                // re-starting after a stop
                savThis.startTime = 0;
            }
            localTime = time - savThis.startTime; // handles local looping
            if (localTime > savThis.dur + savThis.delay && savThis.dur > 0 && savThis.loop) {
                // dur = 0 go forever
                savThis.startTime = time; // we will re-start
                localTime = 0; // this will force re-start at frame 0
            }
            t = 0;
            if (localTime > savThis.delay) {
                // repeat initial frame if there is a delay to start
    ...
    Drag2D = function (cangoGC, grabFn, dragFn, dropFn) {
        ...
        this.grab = function (evt) {
            ...
            this.cgo.cnvs.onmouseup = function (e) {
                savThis.drop(e);
            };
            ...
            this.grabOfs = { x: csrPosWC.x - this.dwgOrg.x, y: csrPosWC.y - this.dwgOrg.y };

            if (this.grabCallback) {
                this.grabCallback(csrPosWC); // call in the scope of dragNdrop object
            }
            this.cgo.cnvs.onmousemove = function (e) {
                savThis.drag(e);
            };
            if (event.preventDefault) {
                // prevent default browser action (W3C)
                event.preventDefault();
            } // shortcut for stopping the browser action in IE
            else {
            ...
            for (i = 0; i < segs.length; i++) {
                ...
                switch (cmd) {
                    ...
                    case 'Q':
                        ...
                        break;
                    case 'q':
                        commands.push('Q', [x + xScale * coords[0], y + yScale * coords[1], x + xScale * coords[2], y + yScale * coords[3]]);
                        px = x + xScale * coords[0];
                        py = y + yScale * coords[1];
                        x += xScale * coords[2];
                        y += yScale * coords[3];
